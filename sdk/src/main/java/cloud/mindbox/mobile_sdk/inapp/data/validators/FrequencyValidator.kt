    ...
    override fun isValid(item: FrequencyDto): Boolean {
        return when (item) {
            is FrequencyDto.FrequencyOnceDto -> {
                val isValid = (item.type == FREQUENCY_TYPE_ONCE && item.kind.lowercase().equalsAny(FREQUENCY_KIND_LIFETIME, FREQUENCY_KIND_SESSION))
            ...
            is FrequencyDto.FrequencyPeriodicDto -> {
               ...
               val isValid =  (item.type == FREQUENCY_TYPE_PERIODIC && item.unit.lowercase().equalsAny(
