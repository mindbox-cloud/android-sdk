...
internal class SnackbarInAppViewHolder(
    ...
    private fun addUrlSource(currentRoot: ViewGroup, layer: Layer.ImageLayer) {
        InAppImageView(currentRoot.context).also { inAppImageView ->
            ...
            when (layer.source) {
                is Layer.ImageLayer.Source.UrlSource -> {
                    ...
                    Glide
                        ...
                        .centerCrop()
                        .into(inAppImageView)
                    //showImageByUrl(url = layer.source.url, imageView = inAppImageView)
                }
            }
            currentDialog.addView(inAppImageView)
            inAppImageView.updateView(wrapper.inAppType, currentDialog)
        }
    }
        ...
        wrapper.inAppType.elements.forEach { element ->
            when (element) {
                is Element.CloseButton -> {
                    ...
                    inAppCrossView.updateView(wrapper.inAppType, currentDialog)
                }
            }
        }
