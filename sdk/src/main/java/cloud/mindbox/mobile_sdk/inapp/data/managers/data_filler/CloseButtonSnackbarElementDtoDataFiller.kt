...
internal class CloseButtonSnackbarElementDtoDataFiller : DataFiller<ElementDto.CloseButtonElementDto?> {
    internal companion object {
        ...
        private const val DEFAULT_SNACKBAR_WIDTH_SIZE = 16.0
        private const val DEFAULT_SNACKBAR_HEIGHT_SIZE = 16.0
        ...
        val newSize =
            if (item.size != null && sizeNames.contains(item.size.kind)) {
                ...
                ElementDto.CloseButtonElementDto.SizeDto(
                    height = DEFAULT_SNACKBAR_HEIGHT_SIZE,
                    kind = DEFAULT_SIZE_KIND,
                    width = DEFAULT_SNACKBAR_WIDTH_SIZE
                )
            }
        return item.copy(
            color = newColor,
